...
while( i < length(theVoices))
	...
	song.durations = theVoices(i).durations*seconds_per_pulse;
    %multiplied them to control the speed of entire sound by bpm
	song.keys=theVoices(i).noteNumbers;
    %NOTES: 52 51 52 47 48 ... 31
    for kk=1:length(song.keys)
		keynum = song.keys(kk);
        disp(keynum);
        tone=key2note(1,keynum,song.durations(kk));
        %recevied keynum, time and vector layer number
		n1 = round(theVoices(i).startPulses(kk)*seconds_per_pulse * fs);
        %rounding up, because index number must be integer
		n2 = n1 + length(tone) - 1;
